...
public class ModalListener extends ListenerAdapter
    ...
    public void onModalInteraction(@NotNull ModalInteractionEvent event)
        ...
        if (event.getModalId().equals("zeusModal"))
            ...
            Category category=event.getGuild().getCategoryById(862076623079211059L);
            
            
            
            //Optional<Member> memberOptional = event.getGuild().getMembersByName(missionName, true).stream().findFirst();

            category.createTextChannel(dateYear+"-"+missionName).addPermissionOverride(event.getGuild().getRoleById(869528487944413194L),EnumSet.of(Permission.VIEW_CHANNEL), null).queue(e ->{
            ...
            if (newChannel!=null)
                ...
                newChannel.sendMessage(missionName+"\n"+missionType+"\n"+date+"\n"+description).queue();
